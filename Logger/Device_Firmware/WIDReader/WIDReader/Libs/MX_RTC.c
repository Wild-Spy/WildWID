...
void sPrintDateTime (char* __s, struct RTCTime* pTime, char* lbl) {
	if (lbl[0] == '\0')
...
void ZipTime(struct RTCTime* rawTime, uint8_t* zipTime, uint8_t Flags) {
...
uint8_t UnzipTime(struct RTCTime* rawTime, uint8_t* zipTime) {
	//using compression method B:
	//							Byte 1
	//Bit 7	Bit 6	Bit 5	Bit 4	Bit 3	Bit 2	Bit 1	Bit 0
	//Yr1	Yr0		Sec5	Sec4	Sec3	Sec2	Sec1	Sec0
