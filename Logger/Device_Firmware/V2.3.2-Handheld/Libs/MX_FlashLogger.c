...
uint8_t RecordToFlash(RTC_Time_t* mTime, uint32_t ID, uint8_t Flags, uint8_t RSSI, uint8_t Activity, uint8_t loggerShortId) {
	...
	if (FLASH_OFFSET_START+RecordCount*FLASHRECORD_SIZE + FLASHRECORD_SIZE <= SerFlash_MAXADDRESS) {
		//RTC_Time_t tmpTime1, tmpTimeAdd;

		Flags &= 0b00000011;
		ZipTime(mTime, tZip, Flags);
		SerFlash_WriteBytes((uint32_t)(FLASH_OFFSET_START+RecordCount*FLASHRECORD_SIZE), 5, tZip);
		//SerFlash_WriteBytes((uint32_t)(RecordCount*FLASHRECORD_SIZE+5), 4, (uint8_t*)ID);
		SerFlash_WriteBytes((uint32_t)(FLASH_OFFSET_START+RecordCount*FLASHRECORD_SIZE+5), 4, (uint8_t*)(&ID));
		SerFlash_WriteBytes((uint32_t)(FLASH_OFFSET_START+RecordCount*FLASHRECORD_SIZE+9), 1, &RSSI);
		SerFlash_WriteBytes((uint32_t)(FLASH_OFFSET_START+RecordCount*FLASHRECORD_SIZE+10), 1, &Activity);
		SerFlash_WriteBytes((uint32_t)(FLASH_OFFSET_START+RecordCount*FLASHRECORD_SIZE+11), 1, &loggerShortId);
