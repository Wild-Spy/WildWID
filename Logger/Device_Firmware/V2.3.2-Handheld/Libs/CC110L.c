...
static volatile uint16_t timer10ms = 0;		//!<Used to keep track of how many 10ms blocks have passed since the 10ms timer was started.
...
void setup_cc110L( uint8_t (*callback)(uint8_t*, uint8_t) )
{
	...
	cli();
	CC11_GD0_PORT.INTCTRL &= ~(PORT_INT0LVL_gm);		//...
	sei();
...
void cc110L_tx( uint8_t* p_buffer, uint8_t length )
{
	...
	_delay_ms(2);	//Needed otherwise the first timeout below times out. 
					//Ie Wait for GDO0 to go hi never happens!?  Perhaps it happens too quickly
					//without this delay and we miss it?  What if we moced the start Tim10ms to
					//before the strobe?  Well it works now so no need to try that.
