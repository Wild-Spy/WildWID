...
#define SIGN_ON_TIME_SECS 10
uint16_t signOutput_StayOn_Secs = 0;
void TurnSignOn(uint16_t onTime);
void TurnSignOff();
...
ISR(RTC_OVF_vect) {
	...
	secCntr++;
	if (signOutput_StayOn_Secs) {
		signOutput_StayOn_Secs--;
		if (signOutput_StayOn_Secs&0x01) { //if it's an even number
...
void MainLoop_Active_Mode() {
	...
	if (Wakeup_Source & WAKEUP_SOURCE_CC2500) {
		...
		Wakeup_Source &= ~(WAKEUP_SOURCE_CC2500);
		//stat = cc_read_reg(TI_CCxxx0_MARCSTATE);
...
// 		GM862_Unit_Off();
	...
	if (signOutput_StayOn_Secs) {
		PORTA.OUTSET = (1<<0);	//sign enable pin square pin on keyboard connector
	} else {
		//PORTA.OUTCLR = (1<<0)|(1<<1)|(1<<2);	//sign enable pin square pin on keyboard connector
		TurnSignOff();
	}
...
void SetupHardware() {
	...
	PORTA.DIRSET = (1<<0)|(1<<1)|(1<<2);	//Sign enable pin - square pin on keyboard connector (active high)
...
// 	if (cc2500_get_RX_FIFO_Status() > 0) {
// 		if ((CC_PORT.IN & (1<<CC_GD0))) {
// 			//doRFWork(TagDispMode);
...
// 	}	
	...
	if (RF_Cal_Cntr >= RFCALUPDATE) {
		...
		//USART_tx_String_P(&USARTC0, PSTR("RF Work in RTC wake...\r\n"));
		//_delay_ms(2);
		...
		//if (cc2500_get_RX_FIFO_Status() > 0) {
			...
			cc_strobe(TI_CCxxx0_SFRX);
			cc_strobe(TI_CCxxx0_SRX);
		//} else {
		//	cc_strobe(TI_CCxxx0_SCAL);
		//}
		
// 		while (cc_read_reg(TI_CCxxx0_FSCAL1) == 0x3F) {
// 			USART_tx_String_P(&USARTC0, PSTR("NO LOCK!\r\n"));
// 		}
// 		_delay_ms(2);
		
		//USART_tx_String_P(&USARTC0, PSTR("...RF Work done\r\n"));
		//_delay_ms(2);
	}
...
void SendEmailChecks() {
	...
	if (Iridium_Unit_On(1) >= 0) {
		...
		//TODO[ ]: implement the send email bif for iridium..
// 		if (Iridium_SendEmail() == 0) {
// 			send_success = 1;
				...
				//if (rxPacket[0] == 0x12 && rxPacket[1] == 0x34) {
					...
					*(((uint8_t*)&rxData)+3) = rxPacket[2];
					...
					if (setting_LoggingEnabled == 2 || (setting_LoggingEnabled == 3 && passFilter)) {
						saveRecord(&tmpTime, rxData, RSSI);
						TurnSignOn(SIGN_ON_TIME_SECS);
					}
					...
					if (passFilter) {
						if (dispLvl >= RF_DISP_TAGID) {
							...
							sprintf_P(s, PSTR("RX ID: 0x%08lX, RSSI: %d"), rxData, (RSSI));
							USART_tx_String(&USARTC0, s);
 ...
 */
uint8_t saveRecord(RTC_Time_t* mTime, uint32_t ID, uint8_t RSSI) {
	...
	//} else {
		...
		if (RecordToFlash(mTime, ID, 0x03, RSSI) == 0) {
			return 2;
		}
 ...
 */
uint8_t loadRecord(uint32_t RecordNo, RTC_Time_t* mTime, uint32_t* ID, uint8_t* RSSI) {
	uint8_t tZip[5];
...
*/
uint8_t RecordToFlash(RTC_Time_t* mTime, uint32_t ID, uint8_t Flags, uint8_t RSSI) {
	uint8_t tZip[5];
