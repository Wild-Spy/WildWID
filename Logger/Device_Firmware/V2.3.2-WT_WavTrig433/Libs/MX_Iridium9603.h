...
#define IRIDIUM_PWR_ON()			do { IRIDIUM_PWR_CTRL_PORT.OUTSET = IRIDIUM_PWR_CTRL_PIN_bm; /*Setup the USART 19200*/ USART_Setup(&IRIDIUM_USART_PORT, &IRIDIUM_USART, 609, -4, 0);} while(0)
#define IRIDIUM_PWR_OFF()			do { IRIDIUM_PWR_CTRL_PORT.OUTCLR = IRIDIUM_PWR_CTRL_PIN_bm; IRIDIUM_USART_PORT.DIRCLR = (1<<2)|(1<<3);} while(0)
...
#define IRIDIUM_ONOFF_ON()			do { IRIDIUM_ONOFF_PORT.OUTSET = IRIDIUM_ONOFF_PIN_bm; } while(0)
#define IRIDIUM_ONOFF_OFF()			do { IRIDIUM_ONOFF_PORT.OUTCLR = IRIDIUM_ONOFF_PIN_bm; } while(0)
	
#define IRIDIUM_SBD_MESSAGE_LEN_MAX	340
...
#define	Iridium_SendFormatedString_P(cmdstrP, replacementText)		do { \
																		sprintf_P(Iridium_tmpStr1, cmdstrP, replacementText); \
																		USART_tx_String(&IRIDIUM_USART, Iridium_tmpStr1); \
																	} while (0)
#define	Iridium_SendFormatedString_PP(cmdstrP, replacementTextP)		do { \
																		stringCopyP(replacementTextP, s); \
																		sprintf_P(Iridium_tmpStr1, cmdstrP, s); \
																		USART_tx_String(&IRIDIUM_USART, Iridium_tmpStr1); \
																	} while (0)	
#define	Iridium_SendFormatedStringAndCheckStart_PP(cmdstrP, replacementTextP, responseStrP, retPtr, timeout, retVal)		\
																	do { \
																		stringCopyP(replacementTextP, s); \
																		sprintf_P(Iridium_tmpStr1, cmdstrP, s); \
																		retVal = Iridium_SendCmdAndCheckStart(Iridium_tmpStr1, responseStrP, retPtr, timeout); \
																	} while (0)	
#define	Iridium_SendFormatedStringAndCheckStart_P(cmdstrP, replacementText, responseStrP, retPtr, timeout, retVal)		\
																	do { \
																		sprintf_P(Iridium_tmpStr1, cmdstrP, replacementText); \
																		retVal = Iridium_SendCmdAndCheckStart(Iridium_tmpStr1, responseStrP, retPtr, timeout); \
																	} while (0)	
