...
extern uint8_t EnterGM862BridgeMode;			
extern uint8_t EnterGM862ConfigMode;	
extern char EmailToAddress[60];
extern char GPRS_APN[30];
extern char GPRS_UserId[30];
extern char GPRS_Passw[30];
...
SerCom_err_t Ser_CMD_Get_AllRecordsFast(USART_t* m_USART) {
	...
	USART_Setup(&PORTC, m_USART, 75, -6, 0);
	...
	if ((tmpByte == 'g') && (tmpSerComErr == SerCom_err_Success)) {
		...
		for (ii = 0; ii < RecordCount; ) {
			...
			for (j = 0; j < recPerGrp; j++) {
				if (USART_USB_IN) {
					tmpFlag = loadRecord(ii+j, &tmpTime, &tmpID);
...
SerCom_err_t Ser_CMD_Cmd_SaveDummy_Record(USART_t* m_USART) {
	...
	if (RecordToFlash(&tmpTime, tmpID, tmpFlags) == 1) {
		tmpAdd = RecordCount;
	} else {
...
SerCom_err_t Ser_CMD_Get_EntryAtAddress(USART_t* m_USART) {
	...
	tmpFlags = loadRecord(tmpAdd, &tmpTime, &tmpID);
...
SerCom_err_t Ser_CMD_Set_DateTime(USART_t* m_USART) {
	...
	SAVE_SYS_MSG(SYS_MSG_UPDATED_TIME, SYS_MSG_UPDATE_TIME_SRC_PC);
	return tmpSerComErr;
}

SerCom_err_t Ser_CMD_Get_RFChan(USART_t* m_USART) {
	return Ser_TxSeg(m_USART, 1, (uint8_t*)&myRF_Chan);
}

SerCom_err_t Ser_CMD_Set_RFChan(USART_t* m_USART) {
	SerCom_err_t tmpErr;
	tmpErr = Ser_RxSeg(m_USART, 1, (uint8_t*)&myRF_Chan);
...
SerCom_err_t Ser_CMD_Get_RFID(USART_t* m_USART) {
	return Ser_TxSeg(m_USART, 5, (uint8_t*)&myRF_ID);
}

SerCom_err_t Ser_CMD_Set_RFID(USART_t* m_USART) {
	SerCom_err_t tmpErr;
	tmpErr = Ser_RxSeg(m_USART, 5, (uint8_t*)&myRF_ID);
	if (tmpErr == SerCom_err_Success) {
		RFIDTOEEPROM();
		NRF_POWERDOWN;
		NRF24L01_SPI_Setup();
...
SerCom_err_t Ser_CMD_Get_RFFilter(USART_t* m_USART) {
	return Ser_TxSeg(m_USART, 8, (uint8_t*)&Filter);

}

SerCom_err_t Ser_CMD_Set_RFFilter(USART_t* m_USART) {
	SerCom_err_t tmpErr;
	tmpErr = Ser_RxSeg(m_USART, 8, (uint8_t*)&Filter);
	if (tmpErr == SerCom_err_Success) {
		WRITEFILTEREEPROM();
...
SerCom_err_t Ser_CMD_Get_LoggerName(USART_t* m_USART) {
	return Ser_TxSeg(m_USART, 20, (uint8_t*)&LoggerName);
}

SerCom_err_t Ser_CMD_Set_LoggerName(USART_t* m_USART) {
	SerCom_err_t tmpErr;
	tmpErr = Ser_RxSeg(m_USART, 20, (uint8_t*)&LoggerName);
	if (tmpErr == SerCom_err_Success) {
		LOGGERNAMETOEEPROM();
	}
	return tmpErr;
...
SerCom_err_t Ser_CMD_Get_GPRSVals(USART_t* m_USART) {
	...
	tmpErr = Ser_TxSeg(m_USART, 30, (uint8_t*)&GPRS_APN);
	if (tmpErr < 0) return tmpErr;
	
	tmpErr = Ser_TxSeg(m_USART, 30, (uint8_t*)&GPRS_UserId);
	if (tmpErr < 0) return tmpErr;
	
	tmpErr = Ser_TxSeg(m_USART, 30, (uint8_t*)&GPRS_Passw);
	if (tmpErr < 0) return tmpErr;
...
SerCom_err_t Ser_CMD_Set_GPRSVals(USART_t* m_USART) {
	...
	tmpErr = Ser_RxSeg(m_USART, 30, (uint8_t*)&GPRS_APN);
	if (tmpErr < 0) return tmpErr;
	
	tmpErr = Ser_RxSeg(m_USART, 30, (uint8_t*)&GPRS_UserId);
	if (tmpErr < 0) return tmpErr;
	
	tmpErr = Ser_RxSeg(m_USART, 30, (uint8_t*)&GPRS_Passw);
	if (tmpErr < 0) return tmpErr;
...
SerCom_err_t Ser_CMD_Get_EmailToAddress(USART_t* m_USART) {
	return Ser_TxSeg(m_USART, 60, (uint8_t*)&EmailToAddress);
}

SerCom_err_t Ser_CMD_Set_EmailToAddress(USART_t* m_USART) {
	SerCom_err_t tmpErr;
	tmpErr = Ser_RxSeg(m_USART, 60, (uint8_t*)&EmailToAddress);
	if (tmpErr == SerCom_err_Success) {
		EMAILTOADDTOEEPROM();
	}
	return tmpErr;
