	...
	if ((tmpByte == 'g') && (tmpSerComErr == SerCom_err_Success)) {
		...
		for (ii = 0; ii < RecordCount; ) {
			...
			for (j = 0; j < recPerGrp; j++) {
				if (USART_USB_IN) {
					tmpFlag = loadRecord(ii+j, &tmpTime, &tmpID, &RSSI, &Activity);
...
SerCom_err_t Ser_CMD_Cmd_SaveDummy_Record(USART_t* m_USART) {
	...
	if (RecordToFlash(&tmpTime, tmpID, tmpFlags, (uint8_t)tmpRSSI, tmpActivity) == 1) {
		tmpAdd = RecordCount;
	} else {
...
SerCom_err_t Ser_CMD_Get_EntryAtAddress(USART_t* m_USART) {
	...
	tmpFlags = loadRecord(tmpAdd, &tmpTime, &tmpID, &tmpRSSI, &tmpActivity);
