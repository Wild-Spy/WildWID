...
void Iridium_BridgeMode() {
	...
	char data[] = "000test123This is some binary data.000";
...
Iridium_err_t Iridium_SendMessageTo9602(char* buf, uint16_t len)
{
	...
	//return's Iridium_err_Success on success or InvalidData on failure (ie bad checksum, invalid number of bytes, etc.)
}
...
Iridium_err_t Iridium_SendMOMessage(char* data, uint16_t len)
{
	...
	if (Iridium_SendMessageTo9602(data, len) != SerCom_err_Success)
	{
