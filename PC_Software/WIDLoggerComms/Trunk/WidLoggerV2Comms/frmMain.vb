...
Public Class frmMain
 ...
 hTemplateFile As IntPtr) As IntPtr
    ...
    Private Sub setupRFParamsComboBoxes()
        ...
        InterLoggerRFParamNames.Add("433MHz WID (2.4kbps GFSK)")
        InterLoggerRFParamValues.Add("0xD3 0x91 0x06 0x04 0x04 0x00 0x04 0x06 0x00 0x10 " + vbCrLf +
                                     "0xA7 0x62 0xF6 0x83 0x13 0x22 0xF8 0x15 0x07 0x30 " + vbCrLf +
                                     "0x18 0x17 0x6C 0x03 0x40 0x91 0x56 0x10 0xE9 0x2A " + vbCrLf +
                                     "0x00 0x1F 0x81 0x35 0x09 0xC0 0x00 0x00 0x00 0x00 " + vbCrLf)
    ...
    Private Function addRetrievedLogsToListBox() As Boolean
        ...
        Dim useLoggerId As Boolean = False
        ...
        Dim LoggerShortIds As List(Of Byte)
        ...
        For i = 0 To times.Count - 1
            ...
            Else
                ...
                End If
                If useLoggerId = True Then
                    Try
                        n.SubItems.Add(LoggerTable(LoggerShortIds(i)))
                    Catch
    ...
    Private Sub btnReadLog_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReadLog.Click
        ...
        If addRetrievedLogsToListBox() = False Then
    ...
    Private Sub bntAddDummyLogs_Click(sender As Object, e As EventArgs) Handles bntAddDummyLogs.Click
        Dim retVals() As Object = Nothing
        Dim tmpStr As String
        Dim useRSSI As Boolean = False
        Dim lvLogItems As New List(Of ListViewItem)
        ...
        Dim itemCount As Long = 101225

        For i = 0 To itemCount
            n = New ListViewItem()
            n.Text = i
            ' There are 10 million ticks in a second
            Dim t As New DateTime(DateTime.Now.Ticks - (itemCount * 3600L * 10000000L))
            n.SubItems.Add(t.ToString("dd/MM/yyyy"))
            n.SubItems.Add(t.ToString("HH:mm:ss"))
            ...
            '    lvSysEventItems.Add(n)
            'Else
            n.SubItems.Add(CType(Rnd() * &HFFFFFFFFL, Long).ToString("X8"))
            n.SubItems.Add("Start") ' (03)")
            'If Flags(i) = 3 Then
            '    n.SubItems.Add("Start") ' (03)")
            'ElseIf Flags(i) = 1 Then
...
FinishSub:
    ...
    Private Sub btnSaveLog_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveLog.Click
        ...
        Dim outLines(lvLogs.Items.Count) As String
        ...
        If lvLogs.Items.Count > 0 Then
            ...
            For Each n In lvLogs.Items
                If recNo Mod 1000 = 0 Then
                    pBTransfer.Value = recNo / lvLogs.Items.Count * 100
                    lblRecCount.Text = recNo.ToString() & "/" & lvLogs.Items.Count.ToString()
                    lblTransfrRate.Text = "..."
                    lblTimeRemain.Text = "..."
                    Application.DoEvents()
                End If
                If n.Selected = True Or useAll = True Then
                    ...
                    outLines(recNo) = tmpLine
                End If
            Next

            If saveDialog.ShowDialog() = Windows.Forms.DialogResult.OK Then
                Dim file As System.IO.StreamWriter
                file = My.Computer.FileSystem.OpenTextFileWriter(saveDialog.FileName, False)
                For Each tmpLine In outLines
...
errExit:
    ...
    'Private Sub bntAddDummyLogs_Click(sender As System.Object, e As System.EventArgs) Handles bntAddDummyLogs.Click
    ...
    '    Dim logsToAdd As Integer = 900000

    '    For i = 1 To logsToAdd
    '        tmpBufDateTime.Add(New DateTime(2016, 8, 8))
    '        tmpBufID.Add(1234)
    '        tmpBufFlags.Add(0)
    '        tmpBufRSSI.Add(-50)
    '        tmpBufActivity.Add(0)
    '        tmpBufLoggerShortId.Add(0)
    '    Next

    '    ReDim mretVals(5)
    '    mretVals(0) = tmpBufDateTime
    '    mretVals(1) = tmpBufID
    '    mretVals(2) = tmpBufFlags
    '    mretVals(3) = tmpBufRSSI
    '    mretVals(4) = tmpBufActivity
    '    mretVals(5) = tmpBufLoggerShortId

    '    Dim ltAry As HashSet(Of String) = New HashSet(Of String)

    '    ltAry.Add(Convert.ToString(0, 16).ToUpper())
    '    ltAry.Add(Convert.ToString(1, 16).ToUpper())

    '    ReDim mretVals_loggerTable(1)
    '    mretVals_loggerTable(0) = ltAry
