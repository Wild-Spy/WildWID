...
Public Class tagParser
    ...
    Public Sub chrtMovingAvg(chrt As Chart, Optional clearData As Boolean = True, Optional WindowSize As Integer = 50)
        ...
        For i = 0 To tagList.Count - 1
            tmpListData = genDataForMovingAvg(i, WindowSize)
        ...
        For i = 0 To tagList.Count - 1
            tmpListData = genRSSIDataForMovingAvg(i, WindowSize)
    ...
    Private Function genRSSIDataForMovingAvg(tagId As Integer, Optional windowSize As Integer = 50, Optional IgnoreMissed As Boolean = True) As List(Of PointF)
        Dim retVal As New List(Of PointF)
        Dim sum, i As Integer
        Dim origWindSize As Integer = windowSize

        If windowSize >= LogCollection.Count Then Return Nothing

        sum = 0
        For i = 0 To origWindSize - 1
            If LogCollection(i).Tags(tagId) = -1 And IgnoreMissed = True Then
                'not picked up.. remove 1 from windowsize
                windowSize -= 1
            Else
        ...
        For i = origWindSize To LogCollection.Count - 1
            retVal.Add(New PointF(i - origWindSize / 2, (sum / windowSize)))
            'retVal.Add(New PointF(i - windowSize / 2, IIf(LogCollection(i - windowSize / 2).Tags(tagId) = -1, 0, 1)))
            'retVal.Add(New PointF(i - windowSize / 2, IIf(LogCollection(i - windowSize / 2).Tags(tagId) = -1, -500, LogCollection(i - windowSize / 2).TagsRSSI(tagId))))
            If LogCollection(i - origWindSize).Tags(tagId) = -1 And IgnoreMissed = True Then
                'not picked up.. add one to window size next time
                windowSize += 1
            Else
                sum -= LogCollection(i - origWindSize).TagsRSSI(tagId)
            End If
    ...
    Public Sub ParseText(ByVal parseStr As String, Optional TagPeriod As Decimal = 10) ', ByRef lvAdd As ListView)
